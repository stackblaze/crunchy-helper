"""Setup wizard: generates crunchy-helper.env when config is missing. Called automatically by manager."""
...
def _write_env(kubeconfig_path: str, pg_host: str, pg_port: str, pg_host_ip: str,
    ...
    ENV_OUT = SCRIPT_DIR / "crunchy-helper.env"
    try:
    ...
    content = f"""\
# crunchy-helper — {cluster} (namespace: {namespace})
...
def run_setup_wizard():
    """Run interactive setup and write crunchy-helper.env. Uses config.SCRIPT_DIR for paths."""
    print()
    divider()
    print("  crunchy-helper — Environment Setup")
    divider()
    print("  No config found. Generating crunchy-helper.env for your Crunchy PGO cluster.")
    print()
